...
if ($is_v2_embed) {
	...
	$listing_css = file_get_contents($listing_css_path);
	if (false === $listing_css) {
		wp_die(
			esc_html('The v2 staff stylesheet could not be read: ' . $listing_css_path),
			'Staff embed error',
			array('response' => 500)
		);
	}
	...
	echo '<!doctype html><html><head><meta charset="UTF-8"><style data-ve-staff-v2>';
	echo $listing_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Trusted local stylesheet content.
	if (!empty($inline_css)) {
		echo $inline_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Administrator-authored listing CSS.
	}
	echo '</style></head><body>';
	echo $listingpgtitle; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated by WordPress.
	echo $htmlcontent; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated by listing templates.
	echo '</body></html>';
	exit;
}
