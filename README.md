...
Keep mock mode enabled while validating mappings. The settings screen also provides a connection and permission test, a staff dry-run, a raw Azure-data view, and a multi-employee comparison of WordPress fields with their mapped Graph values. These inspection tools never write to either system. Location and department terms have an optional **Azure value** that controls both matching inbound Azure data and converting the WordPress term on outbound writes; a blank value uses the term name. The term editor can fetch every distinct `officeLocation` or `department` currently assigned to directory users and offer those values as suggestions without restricting manual entry. Production and mock activity appears on each staff post, and the full log is automatically retained for 90 days.
