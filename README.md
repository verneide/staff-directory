...
Connection fields on the Azure sync screen save individually through authenticated AJAX when their value changes and the field loses focus. The main save button remains responsible for operation toggles, mappings, and transformation rules. Azure inspection scripts use their source-file modification time for cache busting so newly deployed controls do not continue running stale browser-cached behavior.

Keep mock mode enabled while validating mappings. The settings screen also provides a connection and permission test, a staff dry-run, a raw Azure-data view, and a multi-employee comparison of WordPress fields with their mapped Graph values. The raw view includes company, office, department, title, hire date, business phone, extension attributes, and employee ID data when Microsoft Graph returns them. These inspection tools never write to either system. Location and department terms expose their Azure settings in the taxonomy table and Quick Edit. Their optional **Azure value** controls both matching inbound Azure data and converting the WordPress term on outbound writes; a blank value uses the term name. Locations also have a **Company Name** whose fallback order is Azure value and then taxonomy name. **Sync with Azure** defaults to enabled; disabling it skips staff with that location or department during imports and exports. The term editor can fetch every distinct `officeLocation` or `department` currently assigned to directory users and offer those values as suggestions without restricting manual entry. Production and mock activity appears in a sidebar box on each staff post, and the full log is automatically retained for 90 days.

Create a single-tenant Microsoft Entra app registration and use its directory ID, application ID, and client-secret **value** on the settings screen. No Redirect URI is needed because the integration uses the OAuth client credentials flow rather than browser sign-in. Grant tenant admin consent only for the Microsoft Graph **application** permissions the integration needs: `User.Read.All` enables Azure-to-WordPress user sync, `User.ReadWrite.All` enables WordPress-to-Azure user-field updates, and `ProfilePhoto.ReadWrite.All` enables WordPress-to-Azure photo updates. The connection test stores the granted permissions, reports each available capability, and samples one user to show populated standard and on-premises extension fields as mapping suggestions. When a hybrid directory supplies `onPremisesDistinguishedName`, the integration also derives an `organizationalUnit` value from its OU components. Sample values are displayed only during the administrator’s connection test and are not stored. Permissions are rechecked at least daily while synchronization is enabled; operations without their required permission are skipped and logged without disabling permitted sync directions. Webhooks are optional; when used, configure the displayed REST URL and the same random client-state secret in the Graph subscription.
